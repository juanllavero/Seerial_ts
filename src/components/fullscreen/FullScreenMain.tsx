...
import { selectEpisode, selectLibrary, selectSeason, setLibraries } from 'redux/slices/dataSlice';
...
import { changeFullscreenSection, setCurrentlyWatchingShows, setDominantColor, setHomeImageLoaded, setHomeInfoElement } from 'redux/slices/fullscreenSectionsSlice';
...
function FullScreenMain() {
    ...
    const showImageLoaded = useSelector((state: RootState) => state.fullscreenSection.showImageLoaded);
    const seasonImageLoaded = useSelector((state: RootState) => state.fullscreenSection.seasonImageLoaded);
    const dominantColor = useSelector((state: RootState) => state.fullscreenSection.dominantColor);
                ...
                if (library.type !== "Music"){
                    for (const show of library.series) {
                        ...
                        if (season) {
                            ...
                            if (episode){
                                dispatch(setCurrentlyWatchingShows((prevElements: HomeInfoElement[]) => [...prevElements, {
    ...
    return (
        <>
            ...
            <section className="content">
                {
                    section !== FullscreenSection.Seasons && selectedLibrary !== null? (
                        <>
                            {ShowsView()}
                        </>
                    ) : section !== FullscreenSection.Seasons ? (
                    ...
                    ) : section === FullscreenSection.Seasons && selectedLibrary && currentShow && currentSeason && currentEpisode ?(
                        <>
                            {
