...
export const renderRightPanelContent = () => {
    ...
    const handleScroll = (direction: 'left' | 'right', continueWatching: boolean) => {
      const scrollContainer = continueWatching ? document.getElementById('continueWatchingScroll') : document.getElementById('myListScroll');
        
      if (scrollContainer){
        const scrollWidth = scrollContainer.firstElementChild?.clientWidth || 0;
              ...
              <div>
                <div className="horizontal-scroll-title">
                  ...
                  <div className={`scroll-btns ${canScrollCW ? 'visible' : ''}`}>
                    <button className="svg-button" onClick={() => {handleScroll('left', true)}}>
                    ...
                    <button className="svg-button" onClick={() => {handleScroll('right', true)}}>
                ...
                <div className="container" id="continueWatchingScroll" ref={continueWatchingContainer}>
                  {[...currentlyWatchingShows].splice(0, 15).map((value: {
                    library: LibraryData, 
                    show: SeriesData, 
                    season: SeasonData,
                    episode: EpisodeData
                  }) => (
                    <div className="episode-box" key={value.show.id + "continueWatching"}
                      style={{ maxWidth: `${seriesImageWidth}px`}}>
                        <div style={{cursor: "pointer"}}
                          ...
                          <div className="video-button-image-section">
                            <div className={`loading-element-hover ${value.show.analyzingFiles ? 'loading-visible' : ''}`}style={{ width: `${seriesImageWidth}px`, height: `${seriesImageHeight}px` }}>
                              <span className="spinner"></span>
                            </div>
                            <div className="video-button-hover"
                              style={{ width: `${seriesImageWidth}px`, height: `${seriesImageHeight}px` }}
                              >
                              <div className="series-selection-div" onClick={() => handleSeriesSelection(value.show)}></div>
                      ...
                      <a id="seriesName" title={value.show.name} onClick={() => handleSeriesSelection(value.show)}>
                        {value.show.name}
                      </a>
                      {
                        value.show.seasons && value.show.seasons.length > 1 ? (
                          <span id="episodeNumber">
                            {(() => {
                              const minYear = Math.min(...value.show.seasons.map((season: SeasonData) => Number.parseInt(season.year)));
                              const maxYear = Math.max(...value.show.seasons.map((season: SeasonData) => Number.parseInt(season.year)));
                              return minYear === maxYear ? `${minYear}` : `${minYear} - ${maxYear}`;
                            })()}
                          </span>
                        ) : value.show.seasons && value.show.seasons.length > 0 ? (
                          <span id="episodeNumber">{new Date(value.show.seasons[0].year).getFullYear()}</span>
                        ) : null
                      }
                      <ContextMenu 
                        model={[
                          ...
                          ...(value.library.type === "Shows" || !value.show.isCollection ? [{
              ...
              <div className="scroll-container">
                <div className="horizontal-scroll-title">
                  ...
                  <div className={`scroll-btns ${canScrollML ? 'visible' : ''}`}>
                    <button className="svg-button" onClick={() => {handleScroll('left', false)}}>
                      <svg aria-hidden="true" fill="currentColor" height="18" viewBox="0 0 48 48" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M12.7869 24.3848L31.1716 42.7696L34.0001 39.9411L18.4437 24.3848L34.0001 8.82843L31.1716 6L12.7869 24.3848Z" fill="#FFFFFF"></path></svg>
                    </button>
                    <button className="svg-button" onClick={() => {handleScroll('right', false)}}>
                      <svg aria-hidden="true" fill="currentColor" height="18" viewBox="0 0 48 48" width="18" xmlns="http://www.w3.org/2000/svg"><path d="M35.2132 24.3848L16.8284 42.7696L14 39.9411L29.5564 24.3848L14 8.82843L16.8284 6L35.2132 24.3848Z" fill="#FFFFFF"></path></svg>
                    </button>
                  </div>
                </div>
                <div className="container" id="myListScroll" ref={myListContainer}>
                  {[...currentlyWatchingShows].splice(0, 15).map((value: {
                    ...
                    <div className="episode-box" key={value.show.id + "myList"}
                      style={{ maxWidth: `${seriesImageWidth}px`}}>
                        <div style={{cursor: "pointer"}}
                          onMouseEnter={() => {
                            dispatch(showMenu(true));
